...
const addGameToFav = async (idGame) => {
  console.log(config)
  axios
    .get(axiosStore.URL_API + `/user/games/add/${authStore.getUserId}/${idGame}`, config)
    .then((response) => {
...
const checkListFav = async () => {
  axios.get(axiosStore.URL_API + `/user/games/${authStore.getUserId}`, config).then((response) => {
...
const isFav = computed((idGame) => {
  return USER_GAMES.value.some((game) => game.id === idGame) //NO FUNCIONA HAY QUE REVISARLO
})
</script>
...
<template>
  <div class="card" style="width: 19rem; height: 30rem">
    ...
    <div class="card-body">
      ...
      <button v-if="isFav === true" :href="game.urlPage" class="btn" @click="addGameToFav(game.id)">
        <svg
          width="54px"
          height="54px"
          viewBox="0 0 24 24"
